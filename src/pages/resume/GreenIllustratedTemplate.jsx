...
const GreenIllustratedTemplate = ({ data, onEditSection }) => {
  ...
  return (
    <div className="w-full min-h-screen bg-white relative overflow-hidden" style={{ fontFamily: 'Inter, system-ui, sans-serif' }}>
      {/* Header Section */}
      <div className="bg-green-300 px-4 sm:px-8 py-6 sm:py-8 flex flex-col sm:flex-row sm:items-center sm:justify-between relative group">
        {onEditSection && (
          <button
            onClick={() => onEditSection('personalInfo')}
            className="absolute top-2 right-2 p-1.5 bg-green-600 hover:bg-green-700 text-white rounded-full shadow-md opacity-0 group-hover:opacity-100 transition-opacity z-10"
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:space-x-8 space-y-4 sm:space-y-0">
          ...
          {/* Work Illustration */}
          <div className="w-24 sm:w-32 h-18 sm:h-24 bg-gradient-to-br from-blue-100 to-purple-100 rounded-lg flex items-center justify-center relative mx-auto sm:mx-0">
            {/* Simplified work illustration */}
            <div className="relative">
              {/* Computer screen */}
              <div className="w-12 sm:w-16 h-8 sm:h-10 bg-blue-600 rounded-sm relative">
                <div className="absolute inset-1 bg-white rounded-sm"></div>
              </div>
              {/* Person silhouette */}
              <div className="absolute -bottom-1 sm:-bottom-2 left-1 sm:left-2 w-8 sm:w-12 h-6 sm:h-8 bg-blue-800 rounded-t-full"></div>
              {/* Chat bubbles */}
              <div className="absolute -top-1 sm:-top-2 -right-1 sm:-right-2 w-2 sm:w-3 h-2 sm:h-3 bg-orange-400 rounded-full"></div>
              <div className="absolute -top-0.5 sm:-top-1 right-0.5 sm:right-1 w-1.5 sm:w-2 h-1.5 sm:h-2 bg-blue-400 rounded-full"></div>
            </div>
          </div>
        </div>
      </div>
            ...
            <div className="relative group">
              ...
              <div className="space-y-2">
                {skills.slice(0, 3).map((skill, index) => (
                  <p key={`green-skill-${index}-${skill}`} className="text-gray-700">{skill}</p>
                ))}
              </div>
            </div>
...
export default GreenIllustratedTemplate;
